...
# --- EXACT VIEWPORT SETUP MATCHING IMAGE ---
...
# State tracking initialization matching your look
...
with col2:
    ...
    with m1:
        ...
        st.metric(label="Cash-Flow Borrowing Limit (DSCR Cap)", value=f"x{cash_flow_cap:,.2f}")
    with m2:
        st.metric(label="Maximum Portfolio Eligible Offer", value=f"x{max_eligible_loan:,.2f}")
        st.metric(label="Collateral Asset Capacity (LTV Cap)", value=f"x{asset_cap:,.2f}")

    st.markdown(f"#### ... FINAL APPROVED SANCTION AMOUNT: **x{final_sanction:,.2f}**")
    ...
    # --- PLOTLY EXPRESS ENGINE MATCHING IMAGE VIZ ---
    st.markdown("---")
    ...
    # Compile the side PDF download action
    try:
