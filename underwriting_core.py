...
# --- RESTORED MEMO PDF GENERATION IMPLEMENTATION --- 
...
def generate_sanction_memo_pdf(meta_data, metrics_data, scoring_data, results_data): 
    ...
    pdf.cell(95, 6, f"KYC Compliance Track: {'PASSED' if meta_data.get('kyc_cleared', False) else 'PENDING RECONCILIATION'}", ln=True) 
    pdf.ln(5) 
    ...
    pdf.cell(35, 7, "Status", border=1, ln=True) 
 
    pdf.set_font("Helvetica", "", 10) 
    metrics_rows = [ 
        ("Debt Service Cover (DSCR)", f"{metrics_data.get('dscr', 0.0)}x", ">= 1.25x", "Pass" if metrics_data.get('dscr', 0.0) >= 1.25 else "Review Required"), 
        ("Current Ratio Liquidity", f"{metrics_data.get('cr_ratio', 0.0)}x", ">= 1.20x", "Pass" if metrics_data.get('cr_ratio', 0.0) >= 1.20 else "Review Required"), 
        ("Structural Capital Leverage", f"{metrics_data.get('tol_tnw', 0.0)}x", "<= 3.00x", "Pass" if metrics_data.get('tol_tnw', 0.0) <= 3.00 else "Review Required"), 
        ("Loan To Value Ratio", f"{metrics_data.get('ltv', 0.0)}%", "<= 60.0%", "Pass" if metrics_data.get('ltv', 0.0) <= 60.0 else "Review Required"), 
    ] 
 
    for row in metrics_rows: 
        pdf.cell(65, 7, str(row[0]), border=1) 
        pdf.cell(45, 7, str(row[1]), border=1) 
        pdf.cell(45, 7, str(row[2]), border=1) 
        pdf.cell(35, 7, str(row[3]), border=1, ln=True) 
    ...
    pdf.set_font("Helvetica", "", 10) 
    pdf.cell(0, 6, f"Requested Facility Amount: INR {results_data.get('req_loan', 0.0):,.2f}", ln=True) 
    pdf.cell(0, 6, f"Calculated Cash-Flow Ceiling Cap: INR {results_data.get('cash_flow_cap', 0.0):,.2f}", ln=True) 
    pdf.cell(0, 6, f"Calculated Collateral Cover Cap: INR {results_data.get('asset_cap', 0.0):,.2f}", ln=True) 
 
    pdf.set_font("Helvetica", "B", 11) 
    pdf.ln(2) 
    pdf.cell(0, 8, f"FINAL APPROVED POLICY SANCTION OFFER: INR {results_data.get('final_sanction', 0.0):,.2f}", ln=True) 
    pdf.cell(0, 8, f"RISK BASED PRICING YIELD APPLIED (APR): {round(results_data.get('final_rate', 0.0), 2)}%", ln=True) 
