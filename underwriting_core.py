...
def generate_sanction_memo_pdf(meta_data, metrics_data, scoring_data, results_data, qualitative_5cs=None): 
    ...
    kyc_status = "PASSED" if meta_data.get('kyc_cleared', False) else "FAILED"
    pdf.cell(95, 6, f"KYC Compliance Track: {kyc_status}", ln=True) 
    pdf.ln(5) 
    ...
    ltv_v = metrics_data.get('ltv', 0.0)

    rows = [
        ("DSCR", f"{dscr_v}x", ">= 1.25x", "Pass" if dscr_v >= 1.25 else "Fail"),
        ("Current Ratio", f"{cr_v}x", ">= 1.20x", "Pass" if cr_v >= 1.20 else "Fail"),
        ("TOL / TNW", f"{tol_v}x", "<= 3.00x", "Pass" if tol_v <= 3.00 else "Fail"),
    ...
    pdf.cell(0, 6, f"FINAL APPROVED SANCTION OFFER: INR {float(results_data.get('final_sanction', 0)):,.2f}", ln=True) 
    pdf.cell(0, 8, f"RISK BASED PRICING APR: {round(float(results_data.get('final_rate', 0)), 2)}%", ln=True) 
